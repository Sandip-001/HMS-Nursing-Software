...
import {
  ...
  PHARMACY_ICU_DOCTORS as PHARMACY_ICU_DOCTORS,
  PHARMACY_ICU_ORDERS as PHARMACY_ICU_ORDERS,
  PHARMACY_ICU_WARDS as PHARMACY_ICU_WARDS,
  getBalanceDueValue,
  getDefaultBatch,
  getNetPayableValue,
  getTotalPaidValue,
...
import { defaultColumnVisibility, getPharmacyIpdColumns } from "../../ipd/orders/_components/pharmacy-ipd-columns";
import { PharmacyIpdStat } from "../../ipd/orders/_components/pharmacy-ipd-stats";
import { PharmacyIpdFilters } from "../../ipd/orders/_components/pharmacy-ipd-filters";
import { PharmacyIpdColumnToggle } from "../../ipd/orders/_components/pharmacy-ipd-column-toggle";
import { PharmacyIpdOrdersGrid } from "../../ipd/orders/_components/pharmacy-ipd-orders-grid";
...
export default function PharmacyICUOrdersPage() {
  ...
  function handleSendToBillingDept(orderId: string) {
    ...
    toast.success(`Bill sent to Billing Department for ${current?.patientName ?? "patient"}.`);
  }
    ...
    <div className="min-h-screen ">
      <div className="mx-auto max-w-[1700px] space-y-6">
        <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-2xl font-bold text-slate-800 sm:text-3xl">Pharmacy ICU Orders</h1>
            </div>
            <p className="mt-1 text-sm text-slate-500">Deliver medicines, manage returns, and handle emergency billing.</p>
          </div>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <PharmacyIpdStat icon={<ReceiptText className="h-5 w-5" />} label="Total ICU Orders" value={String(stats.totalOrders)} subtitle={`${stats.totalMedicines} medicines ordered`} tone="blue" />
        ...
        <PharmacyIpdFilters filters={filters} results={filteredOrders.length} doctors={PHARMACY_ICU_DOCTORS} wards={PHARMACY_ICU_WARDS} onChange={updateFilter} onReset={() => setFilters(initialFilters)} name={"ICU"} />

        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <p className="min-w-0 shrink text-sm text-slate-500">
            Showing <span className="font-bold text-slate-800">{filteredOrders.length}</span> pharmacy ICU order{filteredOrders.length !== 1 ? "s" : ""}
          </p>
