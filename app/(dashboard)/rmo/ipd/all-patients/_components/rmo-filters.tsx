// app/(dashboard)/rmo/ipd/all-patients/_components/rmo-filters.tsx
"use client";
import { RefreshCcw, Search, SlidersHorizontal } from "lucide-react";
import { Card, CardContent } from "@/components/ui/card";
import { Input } from "@/components/ui/input";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import type { RmoFilters as RmoFiltersState } from "@/types/rmo/ipd/rmo-types";

interface Props {
  filters: RmoFiltersState;
  results: number;
  wards: string[];
  departments: string[];
  onChange: <K extends keyof RmoFiltersState>(key: K, value: RmoFiltersState[K]) => void;
  onReset: () => void;
}

export function RmoFilters({ filters, results, wards, departments, onChange, onReset }: Props) {
  const active = Boolean(filters.search || filters.ward !== "All" || filters.status !== "All" || filters.department !== "All");
  return (
    <Card className="overflow-hidden border-slate-200 shadow-sm">
      <div className="flex flex-col gap-3 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-blue-50/60 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-sm font-bold text-slate-800">Search & Filter Patients</p>
            <p className="text-xs text-slate-500">Filter by patient, ward, department, or status.</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
      ...
      <CardContent className="p-4">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
          ...
          <Select value={filters.status} onValueChange={(value) => onChange("status", value as RmoFiltersState["status"])}>
