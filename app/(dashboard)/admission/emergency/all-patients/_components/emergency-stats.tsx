// app/(dashboard)/admission-desk/emergency/all-patients/_components/emergency-stats.tsx
...
const tones: Record<Tone, { icon: string; accent: string; surface: string }> = {
  ...
  slate: { icon: "bg-slate-100 text-slate-600", accent: "bg-slate-400", surface: "from-slate-50" },
};

export function EmergencyStat({ label, value, subtitle, icon, tone }: { label: string; value: string; subtitle: string; icon: ReactNode; tone: Tone }) {
