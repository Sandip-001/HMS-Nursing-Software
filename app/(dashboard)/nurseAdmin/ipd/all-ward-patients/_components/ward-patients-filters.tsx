// app/(dashboard)/nurse-admin/ipd/all-ward-patients/_components/ward-patients-filters.tsx
"use client";
import { RefreshCcw, Search, SlidersHorizontal } from "lucide-react";
import { Card, CardContent } from "@/components/ui/card";
import { Input } from "@/components/ui/input";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import type { WardDetailFilters } from "@/types/nurse-admin/ipd/ward-detail-types";

interface Props {
  filters: WardDetailFilters;
  results: number;
  wards: string[];
  onChange: <K extends keyof WardDetailFilters>(key: K, value: WardDetailFilters[K]) => void;
  onReset: () => void;
}

export function WardPatientsFilters({ filters, results, wards, onChange, onReset }: Props) {
    ...
    <Card className="overflow-hidden border-slate-200 shadow-sm">
      <div className="flex flex-col gap-3 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-blue-50/60 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-sm font-bold text-slate-800">Search & Filter Ward Patients</p>
            <p className="text-xs text-slate-500">Filter by patient, ward, or current status.</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
      ...
      <CardContent className="p-4">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
          ...
          <Select value={filters.status} onValueChange={(value) => onChange("status", value as WardDetailFilters["status"])}>
            <SelectTrigger className="h-10 w-full"><SelectValue placeholder="Status" /></SelectTrigger>
            <SelectContent>{["All", "Stable", "Under Observation", "Critical", "Discharged"].map((item) => <SelectItem key={item} value={item}>{item === "All" ? "All Statuses" : item}</SelectItem>)}</SelectContent>
          </Select>
        </div>
      </CardContent>
    </Card>
