// app/(dashboard)/nurse-admin/ipd/_components/nurse-admin-filters.tsx
"use client";
import { RefreshCcw, Search, SlidersHorizontal } from "lucide-react";
import { Card, CardContent } from "@/components/ui/card";
import { Input } from "@/components/ui/input";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import type { NurseAdminPatientFilters } from "@/types/nurse-admin/ipd/nurse-admin-types";

interface Props {
  filters: NurseAdminPatientFilters;
  results: number;
  wards: string[];
  departments: string[];
  onChange: <K extends keyof NurseAdminPatientFilters>(key: K, value: NurseAdminPatientFilters[K]) => void;
  onReset: () => void;
  title?: string;
  subtitle?: string;
}

export function NurseAdminFilters({ filters, results, wards, departments, onChange, onReset, title, subtitle }: Props) {
  const active = Boolean(filters.search || filters.ward !== "All" || filters.acuity !== "All" || filters.department !== "All");
  return (
    <Card className="overflow-hidden border-slate-200 shadow-sm">
      <div className="flex flex-col gap-3 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-blue-50/60 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-sm font-bold text-slate-800">{title ?? "Search & Filter Patients"}</p>
            <p className="text-xs text-slate-500">{subtitle ?? "Filter by patient, ward, acuity, or department."}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
      ...
      <CardContent className="p-4">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
          <div className="relative xl:col-span-2">
            ...
            <Input className="h-10 pl-9" value={filters.search} onChange={(event) => onChange("search", event.target.value)} placeholder="Patient, UHID, IPD ID or bed..." />
          </div>
          <Filter label="Ward" value={filters.ward} items={["All", ...wards]} onChange={(value) => onChange("ward", value)} />
          <Filter label="Acuity" value={filters.acuity} items={["All", "Stable", "Under Observation", "Critical"]} onChange={(value) => onChange("acuity", value as NurseAdminPatientFilters["acuity"])} />
        </div>
      </CardContent>
    </Card>
  );
